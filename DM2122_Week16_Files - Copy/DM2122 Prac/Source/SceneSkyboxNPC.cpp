...
void SceneSkyboxNPC::Init()
{
	...
	//meshList[GEO_NPCW] = MeshBuilder::GenerateOBJ("NPC", "OBJ//NPC_Wheel.obj");
	NPC[0].setCoords(0, 0);
...
void SceneSkyboxNPC::Update(double dt)
{
	if (bounceTime > 0)
	{
...
void SceneSkyboxNPC::Render()
{
	...
	//modelStack.PopMatrix();
	RenderTextOnScreen(meshList[GEO_TEXT], to_string(timer), Color(0, 1, 0), 4, 0, 0);
...
void SceneSkyboxNPC::RenderMesh(Mesh* mesh, bool enableLight)
{
...
void SceneSkyboxNPC::RenderSkybox()
{
	modelStack.PushMatrix();
		///scale, translate, rotate
...
void SceneSkyboxNPC::RenderPlayer()
{
	modelStack.PushMatrix();
...
void SceneSkyboxNPC::RenderRoom()
{
...
void SceneSkyboxNPC::RenderEnviroment()//Put Enviromentobject here ETC Cars tand,station car,booth ,plants, well anything static
{
	modelStack.PushMatrix();
...
void SceneSkyboxNPC::RenderText(Mesh* mesh, std::string text, Color color)
{
...
void SceneSkyboxNPC::RenderTextOnScreen(Mesh* mesh, std::string text, Color color, float size, float x, float y)
{
	if (!mesh || mesh->textureID <= 0)
		return;
	glDisable(GL_DEPTH_TEST);
...
string SceneSkyboxNPC::NPCDialogue() // to edit
{
	bounceTime = 1.0f;
...
void SceneSkyboxNPC::InteractionUpdate(int i)
{
...
bool SceneSkyboxNPC::NearNPC() //To check if player is close to NPC
{
...
void SceneSkyboxNPC::RenderTextBox()
{
	if (SpeakIntro == true)
	{
			modelStack.PushMatrix();
...
void SceneSkyboxNPC::RenderNPCText()
{
		modelStack.PushMatrix();
...
bool SceneSkyboxNPC::DuringConvo()
{
	if (timer > 0)
	{
		return true;
